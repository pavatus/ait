...
public class ServerTardis extends Tardis {
    ...
    public void tick(MinecraftServer server) {
        ...
        if (this.isGrowth() && server.getTicks() % 200 == 0) {
            this.getInteriorWorld().getPlayers().forEach(player -> player.sendMessage(Text.translatable("tardis.message.growth.hint").formatted(Formatting.ITALIC, Formatting.WHITE)));
        }
    }
