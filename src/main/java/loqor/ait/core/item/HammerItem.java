...
public class HammerItem extends SwordItem {
	...
	public ActionResult useOnBlock(ItemUsageContext context) {
		...
		if (!world.isClient() && shouldCrashTardis(hammerUses)) {
			...
			world.playSound(null, consoleBlockEntity.getPos(),
					SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.BLOCKS, 0.25f * hammerUses, 1.0f);
		}
		...
		world.spawnParticles(ParticleTypes.SMALL_FLAME, pos.getX() + 0.5f, pos.getY() + 1.25,
				pos.getZ() + 0.5f, 5 * hammerUses, 0, 0, 0, 0.1f * hammerUses);
		...
		world.playSound(null, consoleBlockEntity.getPos(),
				SoundEvents.ENTITY_GLOW_ITEM_FRAME_BREAK, SoundCategory.BLOCKS, 0.25f * hammerUses, 1.0f);
