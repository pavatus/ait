...
public class RiftTarget implements ClampedModelPredicateProvider {
    ...
    public float unclampedCall(ItemStack itemStack, @Nullable ClientWorld clientWorld,
    ...
    private float getAngle(ItemStack stack, ClientWorld world, int seed, Entity entity) {
        ...
        return !this.canPointTo(entity, globalPos)
                ? this.getAimlessAngle(seed, l)
                : this.getAngleTo(entity, l, globalPos.getPos());
    }
    ...
    @Nullable private ClientWorld getClientWorld(Entity entity, @Nullable ClientWorld world) {
    ...
    private boolean canPointTo(Entity entity, @Nullable GlobalPos pos) {
        return pos != null && pos.getDimension() == entity.getWorld().getRegistryKey()
                && !(pos.getPos().getSquaredDistance(entity.getPos()) < 9.999999747378752E-6);
    }
    ...
    public interface CompassTarget {
        @Nullable GlobalPos getPos(ClientWorld world, ItemStack stack, Entity entity);
    }
}
