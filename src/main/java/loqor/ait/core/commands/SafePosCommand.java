...
public class SafePosCommand {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(literal(AITMod.MOD_ID)
                .then(literal("safe-pos").requires(source -> source.hasPermissionLevel(2))
    ...
    public static int execute(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        DirectedGlobalPos.Cached cached = WorldUtil.locateSafe(DirectedGlobalPos.Cached.create(world, posA, (byte) 0), search, false);
        BlockPos blockPos = cached.getPos();

        Text text = Texts.bracketed(Text.translatable("chat.coordinates",
                blockPos.getX(), blockPos.getY(), blockPos.getZ())
        ).styled((style) -> style.withColor(Formatting.GREEN).withClickEvent(
