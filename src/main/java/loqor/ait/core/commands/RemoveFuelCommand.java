...
public class RemoveFuelCommand {
	...
	public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
		dispatcher.register(literal(AITMod.MOD_ID)
				.then(literal("fuel").then(literal("remove").requires(source -> source.hasPermissionLevel(2))
						.then(argument("tardis", UuidArgumentType.uuid()).suggests(TARDIS_SUGGESTION)
								.then(argument("amount", DoubleArgumentType.doubleArg(0, FuelData.TARDIS_MAX_FUEL))
										.executes(RemoveFuelCommand::runCommand))))));
	}
	...
	private static int runCommand(CommandContext<ServerCommandSource> context) {
		...
		tardis.removeFuel(fuelAmount);
		source.sendMessage(Text.literal("Removed fuel from [" + tardis.getUuid() + "] to: [" + tardis.getFuel() + "au]"), true);
