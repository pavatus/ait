...
public class PlugBoardRenderer<T extends PlugBoardBlockEntity> implements BlockEntityRenderer<T> {
	...
	private <E extends ClientPlayerEntity> void renderLeash(PlugBoardBlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider provider, E holdingEntity) {
		int u;
		matrices.push();
		Vec3d vec3d = new Vec3d(3.5, 130.5, -4.5);//holdingEntity.getLeashPos(tickDelta);
		double d = (MathHelper.lerp(tickDelta, entity.getPos().getY(), entity.getPos().getY()) * ((float)Math.PI / 180)) + 1.5707963267948966;
		...
		for (u = 0; u <= 24; ++u) {
			PlugBoardRenderer.renderLeashPiece(vertexConsumer, matrix4f, j, k, l, q, r, s, t, 0.025f, 0.025f, o, p, u, false);
		}
		for (u = 24; u >= 0; --u) {
			PlugBoardRenderer.renderLeashPiece(vertexConsumer, matrix4f, j, k, l, q, r, s, t, 0.025f, 0.0f, o, p, u, true);
		}
		matrices.pop();
	}
